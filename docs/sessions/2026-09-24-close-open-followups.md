...
- **GitHub Pages hosting and weekly scrape (DECISIONS §13):** the site is served at the Pages root through an Actions-built artifact (`site/` as root, `data/` copied in, `.nojekyll`). Pages load data from `data/sgdata.*` relative to the page, and `server.js` maps `/site/data/*` to `data/*`. `.github/workflows/scrape-and-deploy.yml` runs Mondays 03:17 SGT, commits only `data/sgdata.*`, then deploys. The user must set Pages Source = GitHub Actions. Verified locally: 5 pages load their data with 0 console errors, `npm test` passes and the YAML parses. The workflow itself has not run yet.

...
#### Scenario: No tier covers the price
- **WHEN** a broker's only trading fees for the selected market are conditioned on share prices that exclude the reader's typical share price
- **THEN** that broker is listed under "Incomplete data" with a no-trading-fee reason and is not ranked
...
#### Scenario: Exchange fees not yet scraped
- **WHEN** the SGX stocks product has no exchange-fee figures
- **THEN** its card shows no exchange-fee line and no other figure as a price, and it still links to compare broker costs
